...
class CommentaarGewijzigdProcessorTests(TestCase):
    def setup(self):
        ...
        self.processor = CommentaarGewijzigdProcessor(cursor=self.connector.connection.cursor(),
    ...
    def test_update_commentaar(self):
        self.set_up_assets()

        select_asset_query = "SELECT commentaar FROM assets WHERE uuid = '{uuid}'"
        cursor = self.connector.connection.cursor()

        with self.subTest('commentaar check of the first asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00000453-56ce-4f8b-af44-960df526cb30'))
            result = cursor.fetchone()[0]
            self.assertEqual('notitie voor update', result)
        with self.subTest('commentaar check of the second asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00088892-53a8-4dfc-a2c9-875cab2d7e11'))
            result = cursor.fetchone()[0]
            self.assertEqual('notitie voor aanpassing', result)
        ...
        with self.subTest('commentaar check of the first asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00000453-56ce-4f8b-af44-960df526cb30'))
            result = cursor.fetchone()[0]
            self.assertEqual('notitie na update', result)
        with self.subTest('commentaar check of the second asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00088892-53a8-4dfc-a2c9-875cab2d7e11'))
            result = cursor.fetchone()[0]
