...
class ActiefGewijzigdProcessorTests(TestCase):
    def setup(self):
    ...
    def test_update_actief(self):
        ...
        select_asset_query = "SELECT actief FROM assets WHERE uuid = '{uuid}'"
        cursor = self.connector.connection.cursor()

        with self.subTest('actief check of the first asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00000453-56ce-4f8b-af44-960df526cb30'))
            result = cursor.fetchone()[0]
            self.assertEqual(False, result)
        with self.subTest('actief check of the second asset'):
