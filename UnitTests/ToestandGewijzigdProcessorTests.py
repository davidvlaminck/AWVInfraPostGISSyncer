...
class ToestandGewijzigdProcessorTests(TestCase):
    def setup(self):
        ...
        self.processor = ToestandGewijzigdProcessor(cursor=self.connector.connection.cursor(),
    ...
    def test_update_toestand(self):
        self.set_up_assets()

        select_asset_query = "SELECT toestand FROM assets WHERE uuid = '{uuid}'"
        cursor = self.connector.connection.cursor()

        with self.subTest('toestand check of the first asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00000453-56ce-4f8b-af44-960df526cb30'))
            result = cursor.fetchone()[0]
            self.assertEqual('in-opbouw', result)
        with self.subTest('toestand check of the second asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00088892-53a8-4dfc-a2c9-875cab2d7e11'))
            result = cursor.fetchone()[0]
            self.assertEqual('in-gebruik', result)
        ...
        with self.subTest('toestand check of the first asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00000453-56ce-4f8b-af44-960df526cb30'))
            result = cursor.fetchone()[0]
            self.assertEqual('uit-gebruik', result)
        with self.subTest('toestand check of the second asset'):
            cursor.execute(select_asset_query.replace('{uuid}', '00088892-53a8-4dfc-a2c9-875cab2d7e11'))
            result = cursor.fetchone()[0]
            self.assertEqual('verwijderd', result)
